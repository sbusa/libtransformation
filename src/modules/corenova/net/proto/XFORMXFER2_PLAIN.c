...
THIS = {
	...
	.description =
	"This module implements XFORMXFER2_PLAIN protocol, a hacked version of XFORMXFER",
	...
	.transforms  = LIST ("net:session:protocol::XFORMXFER2_PLAIN -> data:message",
                         "net:session:protocol::XFORMXFER2_PLAIN -> data:message:array")
...
_serverProcessRequest (session_t *session, xformxfer_map_t *map, xformxfer_hdr_t *hdr) {
	...
	if (entry) {
		...
		if ((dataSize = I (Transport)->recv (session->transport,&data,hdr->size)) && I (String)->crc32(data, hdr->size) == hdr->csum) {
			...
			if (!I (Transport)->send (session->transport,(char *)hdr,sizeof (xformxfer_hdr_t))) {
			    DEBUGP (DERROR,"_serverProcessRequest","unable to confirm request");
			    free(data);
			    return FALSE;
			}
						...
						if (entry) {
							...
							if (I (Transport)->send (session->transport,(char *)&hdr,sizeof (hdr))) {
								...
								if (I (Transport)->send (session->transport,(char *)binary->data, binary->size)) {
								    ...
								    DEBUGP(DDEBUG, "clientLoop", "failed to send message data");
								
								}
							} else {
							
							    DEBUGP(DDEBUG, "clientLoop", "failed to send header");
							...
							if(I (Transport)->recv(session->transport, (char**)&pconfirm, sizeof(confirmation))) {
							    ...
							    DEBUGP(DDEBUG, "clientLoop", "received confirmation");
							
							    if(memcmp(&hdr, &confirmation, sizeof(confirmation)) != 0) {
							    
                                    DEBUGP(DERROR, "clientLoop", "invalid confirmation received");
                                    break;
							    ...
							    DEBUGP(DERROR, "clientLoop", "unable to received confirmation");
							    break;
...
TRANSFORM_NEW (newProtocolTransformation) {
    ...
    TRANSFORM ("net:session:protocol::XFORMXFER2_PLAIN","data:message",protocol2message);
    TRANSFORM ("net:session:protocol::XFORMXFER2_PLAIN","data:message:array",protocol2message);
